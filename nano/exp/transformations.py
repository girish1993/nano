...
class RotateTransform(Transfrom):
    ...
    def _operate(
        ...
        # x_new = np.maximum(0, np.minimum(x_new, self.dimensions.width - 1))
        # y_new = np.maximum(0, np.minimum(y_new, self.dimensions.height - 1))
...
class ImageTransformer:
    _TRANSFROM_MAPPING = {"scale": ScalingTransform, "rotate": RotateTransform}
...
if __name__ == "__main__":
    ...
    img_trnsfrmr = ImageTransformer(
        obj=pic, transform_type="rotate", tranform_factor=np.pi / 6
    )
